...
//TODO maybe add support for CompactEdition SQL
//using System.Data.SqlServerCe;
using System.Runtime.Serialization.Json;
using RestSharp;
using MarkEmbling.PostcodesIO;

namespace Custom.Umbraco.Events
{
    public class LocationEvents : ApplicationEventHandler
    {
        ...
        private void ContentService_Saving(IContentService sender, SaveEventArgs<IContent> e)
        {
            foreach (var content in e.SavedEntities.Where(c => c.ContentType.Alias.Equals("BlogPost")))
            {
                ...
                {
                    LogHelper.Error(typeof(LocationEvents), "1: Error updating JobReference in DB: ", ex);
                }
            }
            ...
            foreach (var content in e.SavedEntities.Where(c => c.ContentType.Alias.Equals("office")))
            {
                ...
                {
                    LogHelper.Error(typeof(LocationEvents), "Error Converting LonLat from postcode", ex);
                    e.Cancel = true;
                }
            }
            //sender.Save(content);
        ...
        private void UpdateCustomColumn(IContent versionToUpdate, string newRef)
        {
            try
            {
                ...
                using (SqlCommand comm = new SqlCommand(sql, conn))
                {
                    ...
                    if (numRows == 1)
                    {
                        using (SqlCommand commUpdate = new SqlCommand("UPDATE cmsDocument SET JobReference = @newRef where versionId = @versionId", conn))
                        {
            ...
            {
                LogHelper.Error(typeof(LocationEvents), "2: Error updating JobReference in DB: ", ex);
            }
